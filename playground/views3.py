...
def minimax2(board, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        value = float('-inf')
        best_moves = []  # List for storing optimal moves
        for col in valid_locations:
            temp_board = [row[:] for row in board]
            drop_piece(temp_board, col, AI)
            new_score = minimax(temp_board, depth-1, alpha, beta, False)[1]
            if new_score > value:
                value = new_score
                best_moves = [col]  # Found a new best score, reset best moves
            elif new_score == value:
                best_moves.append(col)  # Another optimal move with the same value
            alpha = max(alpha, value)
            if alpha >= beta:
                break

        # Randomly choose one of the best moves
        column = random.choice(best_moves) if best_moves else valid_locations[0]
        return column, value
    else:  # Minimizing player
        value = float('inf')
        best_moves = []  # List for storing optimal moves
...
def get_best_move(request):
    ...
    best_col, _ = minimax2(board, 2, float('-inf'), float('inf'), True)
